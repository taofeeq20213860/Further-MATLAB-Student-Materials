function predictorRankings = F04_findBestPredictors_Final()
    ...
    if exist('S02_MedData.mat', 'file') ~= 2
        error('findBestPredictors:UnableToLoadData', ...
            'Unable to locate the data file S02_MedData.mat')
    end
    X = load('S02_MedData.mat');
    if ~isfield(X, 'MedData')
        error('findBestPredictors:MissingData', ...
            'Unknown variable "MedData" in MAT-file.')
    end
...
function [varPairs, tabNames] = getPairs(Xtab)
